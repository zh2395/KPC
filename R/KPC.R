#' Tn with Knn graph
#'
#' Calculate Tn using directed Knn graph.
#'
...
#' @param k a kernel function k(y,y')
#' @param Knn number of nearest neighbor to use
...
KPCCME = function(Y, X = NULL, Z, ky, kx, kxz, eps, tol = 1e-3) {
  Y = as.matrix(Y)
  Z = as.matrix(Z)
  if (is.null(X)) {
    ...
    Ky = kernlab::kernelMatrix(ky,Y)
    tilde_Ky = Ky - rowMeans(Ky) - matrix(colMeans(Ky),nrow(Ky),ncol(Ky),byrow = T) + mean(Ky)
    return(sum((t(Ly)%*%Lz%*%solve(dim(Y)[1]*eps*diag(dim(Lz)[2]) + t(Lz)%*%Lz)%*%t(Lz))^2)/sum(diag(tilde_Ky)))
  }
  X = as.matrix(X)
...
#}
#colnames(surgical)[Varselect_Knn(surgical[,9],surgical[,1:8],NULL,T,1,kernlab::rbfdot(1/(2*median(dist(surgical$y))^2)),1)]
